@php
    $token = $ci_token;
       try {
          /* $token = decrypt($token);*/
           $callbackUrl = \App\CustomerInterfaceToken::where('token', $token)->where('is_verify', 1)->first('callback_url');
       } catch (Throwable $e) {
           $callbackUrl = '';
       }
@endphp
<div class="header-copyright-area">
    <div class="container-fluid">
        <div class="row">
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
                <div class="header_button">
                    <div class="header-logo-wrap">
                        @if(\Route::currentRouteName() != 'CI.service')
                            <a href="{{ !empty($backUrl) ? url($backUrl) : url()->previous() }}" class="back-btn">
                                <i class="fas fa-chevron-left"></i>
                            </a>
                        @endif

                        {{-- Mobile: shows pushed logo --}}
                        <h3 class="app-title-head">
                            @stack('app-title')
                            {{-- @if(\Route::currentRouteName() === 'CI.service')
                                <img class="logo-pushed" src="{{ asset('public/img/logo/banking_services.jpeg') }}" alt="CI logo" width="130px" height="30px;">
                            @endif --}}
                        </h3>

                        {{-- Desktop: shows layout logo --}}
                        {{-- <img class="logo-primary" 
                            src="{{ asset('public/img/logo/banking_services.jpeg') }}" 
                            alt="CI logo" width="130px" height="30px" 
                            style="margin-left:10px;"> --}}
                        <h3 class="logo-primary web-title-head">
                            Banking Services
                        </h3>

                    </div>
                    <a href="javascript:void(0)" class="btn btn-sm btn-danger float-end" style="font-size: 12px!important;font-weight: 500!important;" id="callBackUrlSubmit">
                            <i class="fas fa-sign-out-alt" style="font-size: 14px"></i> <span>Back to Home</span></a>
                </div>
            </div>
        </div>
    </div>
</div>

@push('js')
    <script nonce="{{ app('csp_nonce') }}">
        $('#callBackUrlSubmit').click(function () {
            var token = '{{ $ci_token }}';
            var callbackUrl = '{{ $callbackUrl->callback_url }}';
            $.ajax({
                type: 'POST',
                url: '{{ route('CI.back-to-home') }}',
                data: {
                    'token' : token,
                    'callbackUrl': callbackUrl,
                    '_token': '{{ csrf_token() }}'
                },
                beforeSend: function () {
                    $('.loadingOverlay').removeClass('loader-none');
                },
                success: function (data) {
                    sessionStorage.removeItem('activeMenu');
                    var url = data.callback_url
                    $('.loadingOverlay').addClass('loader-none');
                    window.location.href = url;
                },
                error: function (data) {
                    $('.loadingOverlay').addClass('loader-none');
                    console.error(data);
                },
            });
        });

    </script>
@endpush
