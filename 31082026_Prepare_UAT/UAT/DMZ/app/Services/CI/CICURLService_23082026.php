    ...
    public static function ciCurl($CIFNumber, $product_type)
    {
        ...
        if ($product_type == 2){ //account
            if (!empty($accessToken) && !empty($accessToken['responseCode']) && $accessToken['responseCode'] == 200) {
                ...
                $responseTimeMs = round(($endTime - $msc) * 1000, 2); 

                // cURL built-in time info
                ...
                Log::channel('api_timing')->info('API Response Time', [
                    'product_type'         => 'Account',
                    'http_code'            => $httpCode,
                    'response_time_ms'     => $responseTimeMs,
                    'connect_time_ms'      => round($curlInfo['connect_time'] * 1000, 2),
                    'total_time_ms'        => round($curlInfo['total_time'] * 1000, 2),
                    'namelookup_time_ms'   => round($curlInfo['namelookup_time'] * 1000, 2),
                ]);
            ...
            if (!empty($accessToken) && !empty($accessToken['responseCode']) && $accessToken['responseCode'] == 200) {
                //$restUrl = 'https://pblcon-uat-lb.primebank.com.bd:8443/card-api/v1/card/details/by/cif';
                    ...
                    if (!empty($responseData['data']) && !empty($responseData['data']['cardDetails'])){
                        ...
                        if (!empty($onlyDCData)){
                            foreach ($onlyDCData as $data) {
                                ...
                                $accountNumberValue = $maskedCardNumber;
            ...
            if (!empty($accessToken) && !empty($accessToken['responseCode']) && $accessToken['responseCode'] == 200) {
                //$restUrl = 'https://pblcon-uat-lb.primebank.com.bd:8443/cbs-service/api/v1/getLoanDetails';
                ...
                //$response = file_get_contents(base_path('/app/APIDemoResponse/getLoanDetails.json'));
