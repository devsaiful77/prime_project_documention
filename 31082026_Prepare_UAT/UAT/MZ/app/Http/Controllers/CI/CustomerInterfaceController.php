...
class CustomerInterfaceController extends Controller
{
    ...
    public function tokenValidate(Request $request)
    {
        ...
        if ($validator->fails()) {
            return response()->json([
                'status_code' => '422',
                'status_description' => 'Invalid request parameter-value'
            ], 200);
        }

        // Live
        // $api_credentials = ['username' => 'ShebaAYS', 'password' => '%*AY5a:#!x9568{vsHeBa}=]'];

        // Local
        $api_credentials = ['username' => 'CIPrimeServe', 'password' => '^*CIPrimea:vSeRvE}=)'];
        ...
        if (empty($username) || empty($password)){
            return response()->json([
                'status_code' => '404',
                'status_description' => 'Username or Password is required!',
            ],200);
        }

        // validated username and password

        if ($api_credentials['username'] !== $username || $api_credentials['password'] !== $password){
            return response()->json([
                'status_code' => '404',
                'status_description' => 'Username or Password dose not Match!',
            ],200);
        }
        ...
        $CIToken->cif_number = $request->cif_number;
        $CIToken->email = $request->email;
        $CIToken->mobile_no = $request->mobile_no;
        $CIToken->callback_url = $request->callback_url;
        $CIToken->request_time = date('d-m-Y h:i:s');

        if ($CIToken->save()) {
            return response()->json([
                'status_code' => '000',
                'status_description' => 'Success',
               /* 'global_transaction_id' => $CIToken->global_transaction_id,*/
                'ci_web_url' => url('/').'/CI/service/?CIToken='.encrypt($CIToken->token),
            ],200);
        }
        return response()->json([
            'status_code' => '999',
    ...
    public function service()
    {
        ...
        $tokenEncrypt  = \request()->get('aysToken');
        try {
        ...
        if($data){
            if ($data->is_verify == 0){
                ...
                session()->put('auth_token', $tokenEncrypt);
                $data->is_verify = 1;
                $data->token_validate_time = date('d-m-Y h:i:s');
            ...
            if (!empty(session()->get('auth_token')) && session()->get('auth_token') === $tokenEncrypt){
                return view('BBL_CI.service', ["ci_token" => $tokenEncrypt, 'step1' => 'active']);
            }else{
                return abort(403, 'Unauthorized User!');
            }
        }

        return view('errors.errors_msg')->with('msg', 'Access Token not found!');
    }
    ...
    public function sendBackDetails()
    {
        ...
        if ($check_token) {
            ...
            $sendBackTickets = WForm::with('ciTicketStatus', 'serviceName')
                ...
                ->where('w_form.source', 'like', '%CI%')
                ->join('reference', 'w_form.reference_number', 'reference.reference_number')
                ->where('reference.form_status', 7)
                ->orderBy('w_form.id', 'DESC')
                ->paginate(10);

            $group_name = \Illuminate\Support\Facades\DB::table('group_info')->where('id', 185)->first(['name']);

            $backUrl = url('/').'/CI/service/?aysToken='.$ci_token;
    ...
    public function complaintSendBackStatus()
    {
        ...
        if ($check_token) {
            ...
            $sendBackTickets = Complaint::with('ciTicketStatus', 'serviceName')
                ...
                ->where('complaint.source', 'like', '%CI%')
    ...
    public function accountVerify(Request $request, $product_type, $request_type)
    {
        ...
        if ($check_token) {
            ...
            $response = CICURLService::ciCurl($cif_number, $product_type);
            ...
            if ($accountNumbers) {
                $unit_items = UnitItem::where('unit_items.product_type_id', $product_type)
                    ...
                    ->where('issue_group_workflows.group_info_id', 185);
                ...
                $data = [
                    ...
                    'email_address' => maskEmail($email_address),
                ...
                return redirect()->back()->with('warning', 'No '.$type.' info found against this cif number!');
            }
        }
        return redirect()->back()->with('warning', 'The Token Is Invalid!');
    }
...
// dd($data);
    ...
    public function otpSubmit(Request $request)
    {
        ...
        if ($check_token) {
            ...
            if ($request->request_type == 'complaint') {
                $ticketStatusUrl = url('/').'/CI/complaint-ticket-status?aysToken='.$ci_token;
            } else {
                $ticketStatusUrl = url('/').'/CI/ticket/status/details?aysToken='.$ci_token;
            }
            $CIUrl = '';
            try {
                $encryptOtp = decrypt($findOtp->otp);
                $token = decrypt($ci_token);
                    ...
                    if (!empty($unitItemData) && $issue_from == 'wform') {
                        ...
                        if ($is_ci_cif == 1) {
                            ...
                            if ($cifResponse !== null && isset($cifResponse->status) && $cifResponse->status === 1) {
                                ...
                                $referenceModelName->sub_group_info_id = 386;
                            } else {
                                // dd("else");
                                $referenceModelName->form_status = 0;
                                // Local CI User ID CI & UAT CI
                                $this->audit([ 'reference_number' => $reference_number, 'unit_id' => 1, 'group_id' => 185, 'user_id' => 'ci', 'action' => 'CIF API Calling Failed', 'comments' => 'STP Failed so sent to CI checker', 'isapproved' => 0, 'subgroup_id' => 386]);
                            }
    ...
    public function sendBackTicket($issueId, $refNum, $viewMode)
    {
        ...
        if ($check_token){
            ...
            $backUrl = url('/').'/CI/send-back/details/?aysToken='.$ci_token;
    ...
    public function complaintSendBackTicket($issueId, $refNum, $viewMode)
    {
        ...
        if ($check_token){
            ...
            $backUrl = url('/').'/CI/complaint-sendback-status/?aysToken='.$ci_token;
    ...
    public function CIWFormSubmit(CIWFormRequest $request)
    {
        ...
        if ($check_token && $request->api_response && $request->account_number && $request->product_type) {
            ...
            if ($request->isMethod('post')) {
                ...
                $firstWorkFlow = IssueGroupWorkflow::where('group_info_id', 185)
                ...
                if ($referenceModelName->save()) {
                    ...
                    $referenceModelName->subgroup_id = (!empty($subgroup_id)) ? $subgroup_id : 185;
                    if (!empty($subgroup_id)) {
                        ...
                        $referenceModelName->sub_group_info_id = (!empty($subgroup_info_id->id)) ? $subgroup_info_id->id : 386;
                    }
                    ...
                    if ($product_type == 1 || $product_type == 3) {
                        ...
                        $wformModelName->account_number = $maskedCardNumber;
                    }
                    $wformModelName->save();
                    ...
                    // Local CI Group ID UAT 185 & LIVE 186
                    // Local CI Sub-Group ID UAT 383 & LIVE 710
                    // Local CI User ID UAT CI & LIVE CI
                    $this->audit([ 'reference_number' => $reference_number, 'unit_id' => 1, 'group_id' => 185, 'user_id' => 'ci', 'action' => 'Ticket Logged', 'comments' => '', 'isapproved' => '1', 'subgroup_id' => 386]);
    ...
    public function CIWFormUpdate(CIWFormRequest $request)
    {
        ...
        if ($check_token) {
            ...
            if ($request->isMethod('post')) {
                ...
                $workflowlist = "";
                $workflow = IssueWorkflow::where('issue_id', $unitItemData->id)->first();
                // Local Customer Interface Group ID UAT 185 & LIVE 186
                $firstWorkFlow = IssueGroupWorkflow::where('group_info_id', 185)
                    ->where('issue_workflow_id', $workflow->issue_workflow_id)
                    ->first();
                if ($workflow->flow_type == FlowEnum::REGULAR) {
                ...
                if (!empty($referenceModelName)) {
                    $referenceModelName->unit_id = (!empty($unit_label)) ? $unit_label : 2;
                    $referenceModelName->subgroup_id = (!empty($subgroup_id)) ? $subgroup_id : 185; // CI Group ID UAT 185 & LIVE 186
                    if (!empty($subgroup_id)) {
                        $subgroup_info_id = SubgroupInfo::where('group_info_id', $subgroup_id)->first();
                        $referenceModelName->sub_group_info_id = (!empty($subgroup_info_id->id)) ? $subgroup_info_id->id : 386; // CI Group ID UAT 383 & LIVE 710
                    }
                    $referenceModelName->save();
    ...
    public function feedbackStore(Request $request)
    {
        ...
        $check_token = TokenValidatedService::validatedToken($ci_token);
        if ($check_token) {
            $customer = CustomerInterfaceToken::where('token', decrypt($ci_token))->first();
