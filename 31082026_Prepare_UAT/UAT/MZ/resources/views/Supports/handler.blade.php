    ...
    <tr>
        ...
        <td>
            <div class="input-group">
                <input class="form-control commonSearchBar" name="common_search" value="{{$searchDataForView['cmmn_search']}}" type="text" placeholder="Ticket Number/ Account Number/ Customer Name/ Customer Id/ Product type/ Service Type/ Log date/ Log time/ Status/ Logger" autocomplete="off">
...
@IF($searchDataForView['active_tab'] == "wform")
    <div class="table-responsive" id="handlerid">
        <table class="table table-striped w-auto bordered table-bordered">
            <thead>
            <tr style="background-color: #DFF0D8">
                <th class="text-center">&nbsp;Ticket&nbsp;No</th>
                <th class="text-center">&nbsp;Account/Card&nbsp;No</th>
                ...
                <th class="text-center"><a href="{{$currentUrl.'reference.date'.'='.'w_form.time_and_ext'}}"><i class="{{($existingOrderBy == 'reference.date'.'='.'w_form.time_and_ext') ? $orderByIcon:$orderByDefIcon }}"></i>&nbsp;Log&nbsp;Date & Time</a></th>
                    ...
                    */
                    ...
                    <tr @if($data['segment_priority'] == 1) style="background: #FFBABC" @endif>
						...
						*/
                        ...
                        <td {!! $styleColor !!} class="text-left">{{ $data['customer_id'] }}</td>
            ...
            @ENDIF
             ...
             <tfoot>
                ...
                @IF($wFormDataObj->total() > $wFormDataObj->perPage())
                    <tr><td class="text-right vcenter no-padding-margin-tb" colspan="16">{{ $wFormDataObj->appends
                    ($searchDataForView)->links('vendor/pagination/default') }}</td></tr>
                @ENDIF
                @ENDIF
            </tfoot>
        </table>
        ...
        <table class="table table-striped w-auto bordered">
            ...
            <tfoot>
                ...
                @IF($complaintDataObj->total() > $complaintDataObj->perPage())
                    <tr><td class="text-right vcenter no-padding-margin-tb" colspan="16">{{ $complaintDataObj->appends
                    ($searchDataForView)->links('vendor/pagination/default') }}</td></tr>
                @ENDIF
            @ENDIF
        ...
        <table class="table table-striped w-auto bordered">
            ...
            @IF(!empty($nonCustomerData['data']))
                ...
                @FOREACH($nonCustomerData['data'] as $data)
					...
					*/
                    ...
                    $remaining = floor(!empty($forwardTime) ? $forwardTime : 0) - $queueDurationInMinutes;
                    if ($remaining > 0) {
                        $remaining = $remaining;
                    } else {
                        $remaining = 0;
            ...
            <tfoot>
                ...
                @IF($nonCustomerDataObj->total() > $nonCustomerDataObj->perPage())
                    <tr><td class="text-right vcenter no-padding-margin-tb" colspan="17">{{
                    $nonCustomerDataObj->appends($searchDataForView)->links('vendor/pagination/default') }}</td></tr>
                @ENDIF
            @ENDIF
