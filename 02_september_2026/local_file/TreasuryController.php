...
class TreasuryController extends Controller
{
        ...
        if ($check_token) {
            ...
            if ($accountNumbers) {
                ...
                $data = [
                    ...
                    'cifNumber' => $check_token->cif_number,
    ...
    public function issueFormFieldBPID(Request $request)
    {
        $bpid_data = null;
        $applicant_count = 1;
        if($request->issue_id == getId('AUCTION_REQUEST')){
...
// Auction Print Form
    ...
    public function ticketStatusDetails()
    {
        ...
        if ($check_token) {
            ...
            $group_name = \Illuminate\Support\Facades\DB::table('group_info')->where('id', getId('WEALTH_MANAGEMENT_GROUP'))->first(['name']);
    ...
    public function sendBackTicket($issueId, $refNum, $viewMode)
    {
        $ci_token = \request()->get('CIToken');
        $requestType = \request()->get('request_type');
        $request_type = $requestType == 'BPID' ? getId('BPID') : getId('AUCTION_REQUEST'); 
