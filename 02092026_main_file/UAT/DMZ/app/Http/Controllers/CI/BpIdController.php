...
class BpIdController extends Controller
{
    ...
    public function firstAccountApi(Request $request)
    {
        ...
        $sameIssueRequest = $this->sameIssueRequestCheck($account_number, $w_form_type);
        if ($sameIssueRequest) {
        ...
        $responseCD = getCustomerDetailsService::getCustomerDetails([
            'account_number' => $account_number,
            'accessToken'    => $accessToken
        ]);

        // ── 2. Join Account (joint holders + nominees) ────────────────────────
        $responseAC = getCustomerDetailsService::getJoinAccount([
            'account_number' => $account_number,
            'accessToken'    => $accessToken
        ]);
        ...
        $accountDataArray['customerPhoneHidden'] = true;
        
        $accountDataArray['customerMobile'] = $phone;
	
	$matchedAccount = collect($dataCD['accounts'] ?? [])->firstWhere('accountId', $account_number);

        $accountDataArray['branchName']    = $matchedAccount['branchName']    ?? null;
        $accountDataArray['customerType'] = $matchedAccount['categoryTitle'] ?? null;
        ...
        if (!empty($dataAC['accounts'])) {
            foreach ($dataAC['accounts'] as $index => $account) {
                ...
                $accountDataArray["dob_$i"]        = !empty($account['dob']) ? date('d-m-Y', strtotime($account['dob'])) : null;
