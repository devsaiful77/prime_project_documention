        ...
        <thead>
            <tr>
                ...
                <th class="vcenter text-left"> 
                    <a href="{{ url('/Divisions/add') }}" class="btn btn-primary gradient ajax_page" title="Add" escape="false"> <i class="fa fa-plus"></i> Add</a>
                </th>
                @endif 
            </tr>
        </thead>
            ...
            @FOREACH($tblData as $data)
                <tr>
                    ...
                    <td class="vcenter actions text-left">
                        @IF($data['status'] == '0')
                            <a href="{{ url('/Divisions/status/'.$data['id'].'/1') }}" class="btn btn-info gradient" title="Active" escape="false"> <i class="fa fa-check"></i> Active</a>
                        @ELSEIF($data['status'] == '1')

                        <?php
                            $editUrl = url('/Divisions/edit/'.$data['id']);
                        ...
                        ?>
                                ...
                                <a href="{{ url('/Divisions/status/'.$data['id'].'/0') }}" class="btn btn-danger gradient" title="Inactive" escape="false"> <i class="fa fa-times"></i> Inactive</a>
