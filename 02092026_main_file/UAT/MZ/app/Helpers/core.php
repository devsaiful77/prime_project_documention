...
use App\SMSEmail;
use App\Reference;
use Modules\Requisition\App\Models\RegionSubgroup;

use App\Enum\LIVEIDEnum;
use App\Enum\LOCALIDEnum;
use App\Enum\UATIDEnum;
...
function getUserGroup($id)
{
    $row = DB::table('user_units')
        ...
        ->where('user_units.module', AccessApp::ServiceComplaint)
        ->first();
    if ($row) {
        return $row->subgroup;
    }
...
function getRequUserGroup($id)
{
    $row = DB::table('user_units')
        ->select('user_units.*', 'subgroup_info.name as subgroup')
        ->join('subgroup_info', 'subgroup_info.id', '=', 'user_units.subgroup_info_id')
        ->where('user_units.user_id', $id)
        ->where('user_units.module', AccessApp::Requisition)
        ->first();
    if ($row) {
        return $row->subgroup;
    }
...
function getRequUserGroupName($id)
{
    $row = DB::table('user_units')
...
function getRequUserDeptName($id)
{
    $row = DB::table('user_units')
...
function getRequUserDivName($id)
{
    $row = DB::table('user_units')
...
function getUserRegName($id)
{
    // $row = DB::table('user_units')
    //     ->select('user_units.*', 'requisition_region.name as regName')
    //     ->join('requisition_region', 'requisition_region.id', '=', 'user_units.region_id')
    //     ->where('user_units.user_id', $id)
    //     ->first();
    // if ($row) {
    //     return $row->regName;
    ...
    $row = DB::table('user_units')
        ->select('user_units.*', DB::raw('GROUP_CONCAT(requisition_region.name) as regNames'))
        ->join('requisition_region', DB::raw('FIND_IN_SET(requisition_region.id, user_units.region_id)'), '>', DB::raw('0'))
        ->where('user_units.user_id', $id)
        ->groupBy('user_units.id')
        ->get();

    if ($row->isNotEmpty()) {
        return $row[0]->regNames; // ✅ Return the concatenated region names
    }
    return '';
}
...
function ccMasking($number, $maskingCharacter = '*')
{
    ...
    $start = substr($number, 0, 6);
    $end = substr($number, -4);
    $maskedLength = $length - 10;

    return $start . str_repeat($maskingCharacter, $maskedLength) . $end;
}
...
function issue_breach($reference, $time)
{
    ...
    $issue_email = \Illuminate\Support\Facades\DB::table('reference')
        ...
        ->where('issue_group_workflows.group_info_id', isset($issue->subgroup_id) ? $issue->subgroup_id : '')
        ->where('reference_number', $reference)
        ->first();
    $user_unit = \Illuminate\Support\Facades\DB::table('user_units')
        ->where('subgroup_info_id', isset($issue->subgroup_id) ? $issue->subgroup_id : '')
        ->where('is_email_allow', 1)
        ->orderBy('unit_id')
        ->get();

    if (!empty($issue) && ($issue->form_status != 11)) {
...
function sentSubgroup($reference, $subgroup_id)
{
    ...
    $users_email = \Illuminate\Support\Facades\DB::table('users')
        ...
        ->whereNotNull('users.email') // Ensure email is not NULL
        ->where('users.email', '!=', '') // Ensure email is not empty
...
function getCommentsActionTimeAndCloseComment($reference_no = '')
{
    ...
    $commentData = DB::table('comments')
        ...
        ->first();
    $closeCommentData = DB::table('comments')
        ->select(
            'comments.time AS action_time',
            'comments.comments AS close_comments',
            'comments.action'
        ...
        ->where('comments.reference_number', 'LIKE', $reference_no)
        ->where('comments.action', 'LIKE', '%Close%')
        ->orderBy('comments.id', 'DESC')
        ->first();
    if (!empty($commentData)) {
        $dataForView['action_time'] = $commentData->action_time;
    }
    if (!empty($closeCommentData)) {
        $dataForView['action_time'] = $closeCommentData->action_time;
        $dataForView['close_comments'] = $closeCommentData->close_comments;
    }
    return $dataForView;
}
...
function userRoles()
{
    if (empty($_SESSION['user_roles_name'])) {
        ...
        $rolename = DB::table('role_user')->select('role_user.role_id', 'roles.name AS role_name')->leftJoin('roles', 'roles.id', 'role_user.role_id')->where('role_user.user_id', $user_id)->first();
...
function getCommentsDataHistory($reference_no = '')
{
    $commentData = array();
    $commentDataObj = DB::table('comments')
        ->select(
...
function isIRISApiPush($issue_id = NULL, $group_id = NULL, $unit_id = NULL)
{
    if ($unit_id == 2) {
        $irisWorkflowExists = DB::table('iris_workflows')->where('iris_workflows.issue_id', $issue_id)->where('iris_workflows.group_info_id', $group_id)->where('iris_workflows.status', 1)->count();
        if ($irisWorkflowExists > 0) {
            return true;
        }
    }
    return false;
...
function isIRISInquiry($issue_id = NULL, $group_id = NULL, $unit_id = NULL)
{
    $irisWorkflowExists = DB::table('iris_workflows')->where('iris_workflows.issue_id', $issue_id)->where('iris_workflows.group_info_id', $group_id)->where('iris_workflows.is_inquiry', 1)->count();
    if ($irisWorkflowExists > 0) {
        return true;
    }
    return false;
...
{
    $visibleDigits = 3;
    $totalLength = strlen($phoneNumber);
    $maskedDigits = $totalLength - (2 * $visibleDigits);
    $mask = str_repeat('*', $maskedDigits);
    $visiblePart = substr($phoneNumber, 0, $visibleDigits);
    $maskedPart = substr($phoneNumber, $visibleDigits, $maskedDigits);
    $lastVisiblePart = substr($phoneNumber, -$visibleDigits);
    $maskedPhoneNumber = $visiblePart . $mask . $lastVisiblePart;
...
function maskEmail($email)
{
    ...
    // Mask only the first and last characters of the username
    $maskedUsername = substr($username, 0, 2) . str_repeat('*', strlen($username) - 2) . substr($username, -1);
...
{
    $allSessions = \App\CIUserSession::all();
    $setting = Setting::select('setting_id', 'ci_session_time')->first();

    foreach ($allSessions as $data) {
        $oldTime = Carbon::parse($data->time);
        $nowTime = Carbon::parse(date('Y-m-d H:i:s', strtotime(Carbon::now())));
        $minutesDifference = $oldTime->diffInMinutes($nowTime);
        if ($minutesDifference >= $setting->ci_session_time ?? 10) {
            $data->delete();
        }
    }
}
...
if (!function_exists('validateSpecialChars')) {
    // $charArr = ['<', '>', '"', "'", '|', '&', ';', '(', ')', '[', ']', '{', '}', '!', '@', '#', '$', '%', '^', '*', '_', '=', '+', '?', '/', ':', '~', '`', ',', '.'];
    function validateSpecialChars(array $requestData, array $columns, array $specialChars)
    {
        $errors = [];
        ...
        foreach ($columns as $column) {
            if (isset($requestData[$column])) {
                foreach ($specialChars as $char) {
                    if (strpos($requestData[$column], $char) !== false) {
                        $errors[$column][] = "The $column contains invalid character: '$char'";
                    }
                }
            }
        }
    ...
    function getUserRegion($sub_group_id){
        $region_id = "";

        if(!empty($sub_group_id)){
            $regionSubgroup = RegionSubgroup::where('subgroup_info_id', $sub_group_id)->first(['region_id']);
            if(!empty($regionSubgroup)){
                $region_id = $regionSubgroup->region_id;
                return $region_id;
            }
        }
...
function getId($key)
{
    ...
    if ($value == 'local'){
        $data = defined(LOCALIDEnum::class . '::' . $key)
            ? constant(LOCALIDEnum::class . '::' . $key)
            : '';
    }elseif ($value == 'production'){
        $data = defined(LIVEIDEnum::class . '::' . $key)
            ? constant(LIVEIDEnum::class . '::' . $key)
            : '';
    }elseif ($value == 'uat'){
        $data = defined(UATIDEnum::class . '::' . $key)
            ? constant(UATIDEnum::class . '::' . $key)
            : '';
    }
    return $data;
}
