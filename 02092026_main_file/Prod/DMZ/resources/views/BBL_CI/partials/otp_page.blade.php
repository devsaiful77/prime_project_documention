    ...
    <form id="verifyOtpForm" action="{{ route('CI.otp-submit') }}" method="post">
        ...
        <div class="tab-content" id="pills-tabContent">
			...
			@if($data['is_send_back'] == 0)
                                <button type="button" class="cancelButton sendBackCancelWeb" ata-url="{{ route('CI.account-verify', ['product_type' => $data['product_type'], 'CIToken' => $data['ci_token'], 'request_type' => $data['request_type'] ]) }}" >Cancel</button>
                            @elseif($data['is_send_back'] == 1)
                                <button type="button" class="cancelButton sendBackCancelWeb" data-url="{{ !empty($backUrl) ? $backUrl : url()->previous() }}">Cancel</button>
                            @endif
