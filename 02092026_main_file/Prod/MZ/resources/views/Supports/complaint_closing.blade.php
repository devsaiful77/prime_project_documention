...
<form class="row gy-2 gx-3 align-items-center" id="handler-search">
    <!-- Radio Buttons Group -->
    <div class="col-auto">
        <div class="form-check form-check-inline">
            <input type="radio" name="active_tab" class="form-check-input form-type" id="wform" value="wform"
                {{ $searchDataForView['active_tab'] == 'wform' ? 'checked' : '' }}>
            <label class="form-check-label" for="wform"><strong>Service Request</strong></label>
        </div>
        <div class="form-check form-check-inline">
            <input type="radio" name="active_tab" class="form-check-input form-type" id="complaint" value="complaint"
                {{ $searchDataForView['active_tab'] == 'complaint' ? 'checked' : '' }}>
            <label class="form-check-label" for="complaint"><strong>Complaint</strong></label>
        </div>
        <div class="form-check form-check-inline">
            <input type="radio" name="active_tab" class="form-check-input form-type" id="nonCustomer"
                value="{{ \App\Enum\IssueTypeEnum::NON_CUSTOMER }}"
                {{ $searchDataForView['active_tab'] == \App\Enum\IssueTypeEnum::NON_CUSTOMER ? 'checked' : '' }}>
            <label class="form-check-label" for="nonCustomer"><strong>Non Customer</strong></label>
        </div>
    </div>
    ...
    <div class="col-auto">
        ...
        <select class="form-select" name="service_category" id="service_category">
            ...
            @foreach($serviceCategoryResult as $allSerCategory)
                <option value="{{ $allSerCategory->id }}"
                    {{ $pBServiceCat == $allSerCategory->id ? 'selected' : '' }}>
                    {{ $allSerCategory->name }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-auto">
        <select class="form-select" name="service_type" id="request_type">
    ...
    <!-- Logger Input -->
    <div class="col-auto">
        <input type="text" name="logger" class="form-control" placeholder="Logger"
            value="{{ $searchDataForView['logger'] }}" autocomplete="off">
    </div>

    <!-- Date From Input -->
    <div class="col-auto">
        <input type="text" name="date_from" class="form-control srvDatePicker" placeholder="Log Date From"
            value="{{ $searchDataForView['date_from'] }}" autocomplete="off">
    </div>

    <!-- Date To Input -->
    <div class="col-auto">
        <input type="text" name="date_to" class="form-control srvDatePicker" placeholder="Log Date To"
            value="{{ $searchDataForView['date_to'] }}" autocomplete="off">
    </div>
...
@php
    ...
    $searchDataForView['viewFrom'] = 'compclosing';
    $getUrlQuery = "";
...
<table class="table table-striped bordered mb-0">
    <colgroup>
        <col width="8%">
        <col width="92%">
    </colgroup>
    <tr>
        ...
        <td>
            <div class="input-group">
                <input class="form-control commonSearchBar" name="common_search" value="{{$searchDataForView['cmmn_search']}}" type="text" placeholder="Ticket Number/ Account Number/ Customer Name/ Product type/ Service Type/ Log date/ Log time/ Status/ Logger" autocomplete="off">
                <span class="input-group-btn ml-2">
                    <button class="btn btn-success btnSearch" type="button"><span class="fa fa-search"></span></button>
                </span>
            </div>
        </td>
    </tr>
...
@php
$currentUrl = url('/Supports/complaintClosing/');
    ...
    <table class="table table-striped w-auto bordered">
        <thead>
            <tr style="background-color: #DFF0D8">
                ...
                <th class="text-center"><a href="{{$currentUrl.'reference.date'.'='.'complaint.time_and_ext'}}"><i class="{{($existingOrderBy == 'reference.date'.'='.'complaint.time_and_ext') ? $orderByIcon:$orderByDefIcon }}"></i>&nbsp;Log&nbsp;Date & Time</a></th>
                ...
                <th class="text-center">&nbsp;In&nbsp;Date & Time</th>
                {{-- <th class="text-center">In&nbsp;Time</th> --}}
                <th class="text-center">Duration&nbsp;(D:H:M:S)</th>
                {{-- <th class="text-center">SLA&nbsp;Time&nbsp;(M)</th> --}}
                <th class="text-center">Remaining&nbsp;(M)</th>
        ...
        {{-- @IF(!empty($complaintDataObj['data'])) --}}

            @FOREACH($complaintDataObj->get() as $data)
                @php
                ...
                */
                ...
                if ($issue_flow_type == "regular") {
                    if ($data['unit_id'] == 2) {
                        ...
                        if($highPriority == "High"){
                            $styleColor = "style='font-weight:bold;color:#d26b04;'";
                        }
                ...
                /*$remaining = floor(!empty($QueueSLATime) ? $QueueSLATime : 0) - $queueDurationInMinutes;
                if ($remaining > 0) {
                    $remaining = $remaining;
                } else {
                    $remaining = 0;
                }*/
                $stime = date('Y-m-d H:i:s', (int)$sqlFormQueueInTime);
                $etime = date('Y-m-d H:i:s');
                $duration = $allServiceCategory->queueDurationCalculator($stime, $etime);

                $fixedSlaMinutes = 7 * 8 * 60;
                $QueueSLATime = $fixedSlaMinutes;
                $blinkTime = 480;
                list($d, $h, $m, $s) = explode(':', $duration);
                $queueDurationInMinutes = ($d * 24 * 60) + ($h * 60) + $m;
                $remaining = max(0, $QueueSLATime - $queueDurationInMinutes);

                if (!empty($data['in_date_time']) && $data['in_date_time']['issendback'] > 0) {
                    $styleColor = "style='font-weight:bold;color:#0000FF;'";
                } elseif (trim($data['priority']) === "High") {
                    $styleColor = "style='font-weight:bold;color:#d26b04;'";
                } elseif ($queueDurationInMinutes > $QueueSLATime) {
                    $styleColor = "style='font-weight:bold;color:#FF0000;'";
                } elseif ($queueDurationInMinutes > ($QueueSLATime - $blinkTime)) {
                    $styleColor = "style='font-weight:bold;color:#008000;animation: blinker 1s linear infinite;'";
                }

                ?>

                <tr  @if($data['segment_priority'] == 1) style="background: #FFBABC" @endif>
                    ...
                    <td {!! $styleColor !!} class="text-center">
                        <?php
                        $detailsUrl = url('/Supports/ComplaintClosingDetails/'.encrypt($data['reference_number']));
                    ...
                    <td {!! $styleColor !!} class="text-left">{{ $data['SIF_Number'] }}</td>
                    ...
                    <td {!! $styleColor !!} class="text-center">{{ Carbon::createFromTimestamp($data['date'])->format('Y-m-d') .' '. date('h:i:s A', strtotime($data['time_and_ext'])) }}</td>
                    {{-- <td {!! $styleColor !!} class="text-center">{{ $data['time_and_ext'] }}</td> --}}
                    <td {!! $styleColor !!} class="text-center">{{ \Carbon\Carbon::createFromTimestamp($sqlFormQueueInTime)->format('Y-m-d h:i:s a') }}</td>
                    {{-- <td {!! $styleColor !!} class="text-center">{{ \Carbon\Carbon::createFromTimestamp($sqlFormQueueInTime)->format('h:i:s a') }}</td> --}}
                    <td {!! $styleColor !!} class="text-center">{{ $duration }}</td>
                    {{-- <td {!! $styleColor !!} class="text-center">{{ $QueueSLATime }}</td> --}}
                    <td {!! $styleColor !!} class="text-center">{{ $remaining }}</td>
                    <td {!! $styleColor !!} class="text-center">{{ $status }}</td>
                    {{-- <td {!! $styleColor !!} class="text-center">{{ $data['created_by'] }}</td> --}}
                    <td {!! $styleColor !!} class="text-center">

                        @php
                            $customeComment = DB::table('comments')
                                ->select('comments.reference_number', 'comments.user_id', 'comments.unit_id', 'comments.subgroup_id', 'subgroup_info.name', 'comments.id')
                        ...
                        {{ $data['created_by'] }} ({{$customeComment->name ?? 'Not Found'}})
                    </td>
                    ...
                    <td {!! $styleColor !!} class="text-center">
                        @IF($form_status == 10)
                            ...
                            @can('revokeAssignedRequest')
                                <?php
                                // unassign
                                // $searchUrl = url('/Supports/ComplaintClosingDetails/'.encrypt($data['reference_number']));
...
@php
    $handlerUrl = url('Supports/complaintClosing?');
    $handlerStaticParam = '';
          ...
          @IF(!empty($complaintData['data']))
            @FOREACH($complaintData['data'] as $data)
              ...
              <tr>
                ...
                <td class="text-center">{{ (!empty($data['product_type'])) ? $data['product_type'] : $data['product_type_ext'] }}</td>
                <td class="text-center">{{ (!empty($data['issue_name'])) ? $data['issue_name'] : $data['complaint_type'] }}</td>
                <td class="text-center">{{ \Carbon\Carbon::createFromTimestamp($data['date'])->format('Y-m-d') }}</td>
                <td class="text-center">{{ $data['time_and_ext'] }}</td>
