...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Settings Actions</h5>
    ...
    <table class="table table-bordered table-striped table-hover">
        ...
        <tr>
            ...
            <th scope="col">File Size Upload Limit</th>
        ...
        <tr>
            {{-- <th scope="row">{{ ++$key }}</th> --}}
            <td>{{ $data->session_lifetime }}</td>
            <td>{{ $data->ci_session_time }}</td>
            <td>
                {{ isset($data->file_size_limit) ? $data->file_size_limit / 1024 . ' MB' : 'N/A' }}
            </td>
            <td>{{ $data->password_change_time }}</td>
            <td>@if($data->allow_ip_restriction == 1) <button class="btn-info">{{'Yes'}}</button> @else <button class="btn-danger">{{'No'}}</button> @endif</td>
            <td>{{ $data->sla_blink }}</td>
            <td>{{ $data->sla_email_time }}</td>
            <td>{{ $data->forward_time }}</td>
			<td>@if($data->noncustomersms == 1) <button class="btn-info">{{'Yes'}}</button> @else <button class="btn-danger">{{'No'}}</button> @endif</td>
            ...
            <td class="vcenter actions text-center">
                {{-- @if($row['form_status'] == 0)
                    <a href="{{ url('settings/assign', $row['id']) }}" class="btn btn-primary gradient ajax_page" title="Assign" escape="false"> Assign</a>
                @else
                    @if($row['modified_by'] == Auth::user()->user_id)
                        <a href="#" class="btn btn-info gradient ajax_page" title="Approve" escape="false"> View</a>
                        <a href="{{ url('/settings/approve', $row['id']) }}" class="btn btn-success gradient ajax_page" title="Approve" escape="false"> <i class="fa fa-plus"></i> Approve</a>
                        <a href="{{ url('settings/send-back', $row['id']) }}" class="btn btn-warning gradient ajax_page" title="Send Back" escape="false">SendBack To Maker</a>
                        <a href="{{ url('settings/reject', $row['id']) }}" class="btn btn-danger gradient ajax_page" title="Reject" escape="false">Reject</a>
                ...
                @if($isChecker == false)
                    @if($data['form_status'] == 7)
                        <a href="{{ url('settings/tmp-edit', $data['id']) }}" class="btn btn-success gradient ajax_page" title="Edit" escape="false"> <i class="fa fa-pencil"></i> Edit</a>
                    @endif
    ...
    <script>
        $(document).ready(function(){
            ...
            $('#sendBack-btn').click(function(e){
                ...
                const id = $('#assign_button').data('id');
                let url = "{{ url('settings/send-back', '') }}";
                url += '/' + id;
