    ...
    <table class="table table-bordered table-condensed">
        <thead>
            <tr>
                ...
                <?php
                /*
                <th class="text-center vcenter wordwrap ">Remaining (M)</th>
                */
                ?>
                ...
                <th class="text-center vcenter wordwrap ">Balance</th>
                @endif
                ...
                */
				...
				@endif
                    ...
                    */
					...
					<td class="text-center vcenter no-padding-margin-tb">{{ $data['balance'] ?? '' }}</td>
					@ELSEIF( $searchDataForView["form_type"] == 'complaint')
					<td class="text-center vcenter no-padding-margin-tb"> '{{ $data['account_number'] }}</td>
					<td class="text-center vcenter no-padding-margin-tb">{{ $data['balance'] ?? '' }}</td>
					@ELSEIF( $searchDataForView["form_type"] == 'noncustomer')
                    ...
                    @if($searchDataForView['issueTypeExcel'] == 1)
                        @IF(!empty($extraFieldArr))
                            @if($data['w_form_type'] != 1103 && $data['w_form_type'] != 1105)
                                @FOREACH($extraFieldArr AS $extFld)
                                    @php
                                        $curVal = current($extFld);
                                        if (is_array($curVal) || is_object($curVal)) {
                                            $curVal = '';
                                        }
                                    @endphp
                                    <td style="" class="text-center vcenter no-padding-margin-tb">'{{ $curVal }}</td>
                                @ENDFOREACH
                            @endif
                        @ENDIF
                    @else
                        @IF(!empty($extraFieldArr))
                            @if($data['w_form_type'] != 1103 && $data['w_form_type'] != 1105)
                                @FOREACH($extraFieldArr AS $extFld)
                                    ...
                                    @endphp
                                    <td class="text-center vcenter no-padding-margin-tb">{{ $curKey }}:{{ $curVal }}</td>
                                @ENDFOREACH
                            @else
                                @php
