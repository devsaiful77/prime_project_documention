...
Route::post('CI/testing-url', ['uses' => 'CustomerInterfaceController@callbackUrl'])->name('CI.testing-url');
...
Route::group(['namespace' => 'CI', 'middleware' => ['ci_auth', 'record_last_activity']], function () {
    ...
    // BPID
    Route::get('BPID/service', ['uses' => 'TreasuryController@bpidService'])->name('BPID.service');
    Route::get('BPID/service-type', ['uses' => 'TreasuryController@bpidServiceType'])->name('BPID.service-type');
    Route::post('BPID/printBpIdBtn', ['uses' => 'TreasuryController@printBpIdBtn'])->name('BPID.printBpIdBtn');
    Route::post('BPID/printAuctionRequestBtn', ['uses' => 'TreasuryController@printAuctionRequestBtn'])->name('BPID.printAuctionRequestBtn');

    Route::post('bpid/calling-account-api', ['uses' =>  'BpIdController@firstAccountApi'])->name('bpid.call_first_account_api');


    Route::post('BPID/issue-extra-form', ['uses' => 'TreasuryController@issueFormFieldBPID'])->name('BPID.issue-extra-form');
    Route::get('BPID/ticket-status', ['uses' => 'TreasuryController@ticketStatusDetails'])->name('BPID.ticket-status-details');
    Route::get('BPID/send-back/details', ['uses' => 'TreasuryController@sendBackDetails'])->name('BPID.send-back-details');
    Route::get('BPID/send-back/ticket/{issueId}/{refNum}/{viewMode}', ['uses' => 'TreasuryController@sendBackTicket'])->name('BPID.send-back.ticket');
    ...
    Route::post('CI/complaint-update', ['uses' => 'CustomerInterfaceController@CIComplaintUpdate'])->name('CI.complaint-update')->middleware(['throttle:5,1']);

    /************ Mahdi 30-July-2023 ***********/
    Route::get('CI/issue/conditional/fields/{issue_id}/{id}', ['uses' => 'ConditionalFieldController@issueConditionalFields']);
    Route::get('CI/issue/dependant/fields/{issue_id}/{value}/{id}', ['uses' => 'ConditionalFieldController@issueDependantFields']);
    Route::post('CI/updateWForm', ['uses' => 'CustomerInterfaceController@CIWFormUpdate'])->middleware(['throttle:5,1']);
    Route::get('CI/issue/conditional/{issue_id}', ['uses' => 'ConditionalFieldController@issueConditional']);
});
