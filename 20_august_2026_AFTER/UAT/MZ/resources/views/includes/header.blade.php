...
<div class="d-flex align-items-center main-header-wrap" style="height: 65px; background-color: #03427e;">
    ...
    @cannot('admin')
        ...
        @else
            @php
                $regName = \Modules\Requisition\App\Models\Region::find(Auth::user()->user_unit->region_id ?? null);
            @endphp
            @if($regName)
                <div class="py-3 px-1 bd-highlight">[&nbsp;{{ $regName->name }}&nbsp;]</div>
            @endif
    ...
    <div class="py-3 px-1">
        @if(Auth::user()->roles->pluck('module')->contains(1) && Auth::user()->roles->pluck('module')->contains(2))
            <div class="dropdown dropdown-click-only">
                <a href="#" class="d-flex align-items-center me-2" id="profileDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                    <img src="{{ asset('public/images/default.png') }}" alt="" class="rounded-circle img-thumbnail" width="40" height="40">
                </a>
                <ul class="dropdown-menu p-1 pe-2" aria-labelledby="profileDropdown">
                    <li>
                        <a href="{{ route('redirect.requisition') }}" class="nav-link dropdown-item ">
                            Switch to Requisition
                        </a>
                    </li>

                    <li>
                        <a href="{{ url('/logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="nav-link dropdown-item">
        ...
        @elseif(Auth::user()->roles->pluck('module')->contains(1) || Auth::user()->roles->pluck('module')->contains(2))
