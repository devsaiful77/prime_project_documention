    ...
    <table class="table table-bordered table-condensed">
        ...
        <tr>
            ...
            @if($searchDataForView["form_type"] != 'noncustomer')
                <th style="color: hite" class="text-center vcenter wordwrap ">Card / Acc Number</th>
            @endif
            @if($searchDataForView["form_type"] != 'noncustomer')
...
{{--                <th style="color: white" class="text-center vcenter wordwrap ">Segment Code</th>--}}
            ...
            <th style="color: " class="text-center vcenter wordwrap ">API Updated By</th>
            <th style="color: " class="text-center vcenter wordwrap ">API Update Status</th>
            <th style="color: " class="text-center vcenter wordwrap ">API Update Date</th>
            <th style="color: " class="text-center vcenter wordwrap ">Failed Reason</th>
        </tr>
        </thead>
        <tbody>
        ...
        @IF(!empty($dataForView['data']))
            @FOREACH($dataForView['data'] AS $data)
                ...
                $source_api = \App\UserSubgroupInfo::where('user_subgroup_infos.username', '=', $data['updated_by'])->first(['subgroup_name']);
                //dd($source_api);
...
{{--                            <td class="text-center vcenter no-padding-margin-tb">{{ $data['segment'] }}</td>--}}
                        ...
                        <td class="text-center vcenter no-padding-margin-tb">
                            @if(!empty($data['updated_by'])) {{ $data['updated_by'] }} @endif
                            @if(!empty($source_api->subgroup_name)) {{ ' ('. $source_api->subgroup_name .')' }} @endif
                            {{--{{ !empty($data['updated_by']) ? $data['updated_by'] .' ('. $source_api->subgroup_name .')' : $source_api->subgroup_name }}--}}
            ...
            @ENDFOREACH
        @ELSE
            <tr><th class="text-center vcenter no-padding-margin-tb" colspan="13">Data not available</th></tr>
        @ENDIF
        </tbody>
        <tfoot>
        @IF(!empty($dataObj))
            @IF($dataObj->total() > $dataObj->perPage())
                <tr><td class="text-right vcenter no-padding-margin-tb" colspan="13">{{ $dataObj->appends($searchDataForView)->links('vendor/pagination/default') }}</td></tr>
            @ENDIF
        @ENDIF
        </tfoot>
    </table>
