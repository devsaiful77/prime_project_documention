    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Holidays Actions</h5>
            ...
            @FOREACH($holidayTmp as $data)
                <tr>
                    ...
                    <td class="vcenter text-center">
                        @if (!empty($data['importable_file']))
                            <a target="_blank" href="{{ url('Holidays/download', $data['id']) }}">{{ $data['file_name'] }}</a>
                        @else
                            {{ "No file is requested" }}
                        @endif
                    </td>
                    <td class="vcenter text-center">
                        {{ $data['action'] }}
                    ...
                    <td class="vcenter actions text-center">
                        ...
                        @if($isChecker == false)
                            @if($data['form_status'] == 7)
                                <a href="{{ url('Holidays/tmp-edit', $data['id']) }}" class="btn btn-success gradient ajax_page" title="Edit" escape="false"> <i class="fa fa-pencil"></i> Edit</a>
                            @endif
                                <a href="{{ url('delete/tmp-data', ['id' => $data['id'], 'table' => 'holiday_tmps']) }}" class="btn btn-danger gradient ajax_page" title="Delete" escape="false">
    ...
    <script>
        $(document).ready(function(){
                    ...
                    $.ajax({
                        ...
                        success:function(data){
                            ...
                            $.each(data.new_data, function(key, newValue) {
                                ...
                                var newDataCell = $('<td></td>').html(newValue !== null ? newValue : 'N/A');
            ...
            $('#sendBack-btn').click(function(e){
                ...
                const id = $('#assign_button').data('id');
                let url = "{{ url('Holidays/send-back', '') }}";
                url += '/' + id;
