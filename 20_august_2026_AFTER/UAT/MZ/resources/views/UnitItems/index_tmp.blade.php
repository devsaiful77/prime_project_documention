    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Issues Actions</h5>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Issue Conf Actions</h5>
...
<div class="table-responsive">
    <table class ="commonDataTableAllAsc table table-bordered table-striped table-hover">
        ...
        <tbody style="word-break: break-all;">
            @IF(!empty($issuesTmp))
            @FOREACH($issuesTmp as $data)
                <tr>
                    <td class="vcenter text-center"> {{ $data['name'] }} </td>
                    <td class="vcenter text-center"> {{ $data->productType->name }} </td>
                    <td class="vcenter text-center">
                        @if($data->issues_from == 'wform') service-request @else {{ $data->issues_from }} @endif
                    </td>
                    <td class="vcenter text-center">
                        {{ (!empty($data->issueCategories->name)) ? $data->issueCategories->name : 'N/A' }}
                    </td>
                    <td class="vcenter text-center">
                        @if($data['status'] == 0)
                            Active
                    ...
                    <td class="vcenter text-center">
                        {{ ($data->is_ci_cif==1)?'Yes':'No' }}
                    </td>
                    <td class="vcenter text-center">{{ $data->action }}</td>
                    @if (!$isChecker)
                    ...
                    <td class="vcenter actions text-center">
                        ...
                        @if($isChecker == false)
                            @if($data['form_status'] == 7)
                                <a href="{{ url('Issues/tmp-edit', $data['id']) }}" class="btn btn-success gradient ajax_page" title="Edit" escape="false"> <i class="fa fa-pencil"></i> Edit</a>
                            @endif
                                <a href="{{ url('delete/tmp-data', ['id' => $data['id'], 'table' => 'unit_items_tmp']) }}" class="btn btn-danger gradient ajax_page" title="Delete" escape="false">
    ...
    <script>
        $(document).ready(function(){
            ...
            function checkerTableData(id){
                if(id){
                    $.ajax({
                        url: "fetch/checker-table-issues/" + id,
                        type: "GET",
                        success:function(data){
                            console.log(data);
