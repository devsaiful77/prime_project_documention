...
}  elseif (!empty($dataForView->region_id)) {
  ...
  <div class="col-md-6 offset-md-3 col-12">
    <div class="d-flex">
      ...
      <div class="custom-control custom-radio">
        <input type="radio" id="region" name="type" value="Region" class="custom-control-input green" {{$regionChecked}} required/>
        <label class="custom-control-label p-0" for="region">Requisition Region</label>
      </div>
    </div>
  </div>
...
<div id="showSubgroup">
  ...
  <div class="form-group row">
    <label class="control-label col-md-2 col-sm-2 col-xs-12">Requisition Initiator<span class="required">*</span> </label>
    <div class="col-md-6 col-sm-6 col-xs-12">
      @inject('units','App\Services\UtilService')
      @php
        $unitId = array();
...
<div id="showRegion" style='display: none'>
    <div class="form-group row">
        <label class="control-label col-md-2 col-sm-2 col-xs-12" for="region">For Region User </label>
        <div class="col-md-6 col-sm-6 col-xs-12">
            <select class="form-select select2" name="region_id[]" id="region" multiple="multiple">
                <option value="">Select Region</option>
                    @inject('region','App\Services\UtilService')
                @php
                    //   $regionId = (!empty($dataForView->region_id)) ? $dataForView->region_id : old('region_id');
                    $regionId = [];

                    if (!empty($dataForView->region_id)) {
                        $regionId = explode(',', $dataForView->region_id);
                    } else {
                        $regionId = old('region_id', []);
                    }
                @endphp
                {!! $region->getAllRegions($regionId) !!}
        </select>
    </div>
   ...
   <div class="form-group row">
    <label class="control-label col-md-2 col-sm-2 col-xs-12">Requisition Initiator<span class="required">*</span> </label>
    <div class="col-md-6 col-sm-6 col-xs-12">
      @inject('requUnits','App\Services\UtilService')
      @php
        $requUnitId = array();
        if(!empty(old('unit_id'))) {
          $requUnitId = old('unit_id');
        } elseif(!empty($dataForView->unit_id)) {
          $requUnitId = explode(',',$dataForView->unit_id);
        }
      @endphp
      {!! $requUnits->getAllUnitRequAdmin($requUnitId); !!}
    </div>
    <div class="error">{{ $errors->first('unit_id') }}</div>
  </div><!-- Requ Unit -->
</div>
