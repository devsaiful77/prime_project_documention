    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Branch Code Actions</h5>
...
<div class="table-responsive">
    <table class ="commonDataTableAllAsc table table-bordered table-striped table-hover">
        ...
        <tbody style="word-break: break-all;">
            @IF(!empty($branchTmpData))
            @FOREACH($branchTmpData as $data)
                <tr>
                    ...
                    @if(!$isChecker)
                        <td class="vcenter text-center"> {{ $data['comments'] }} </td>
                    @endif
                    <td class="vcenter text-center">
                        @php
                    ...
                    <td class="vcenter actions text-center">
                        ...
                        @if($isChecker == false)
                            @if($data['form_status'] == 7)
                                <a href="{{ url('branchcode/tmp-edit', $data['id']) }}" class="btn btn-success gradient ajax_page" title="Edit" escape="false"> <i class="fa fa-pencil"></i> Edit</a>
                            @endif
                                <a href="{{ url('delete/tmp-data', ['id' => $data['id'], 'table' => 'branch_code_tmps']) }}" class="btn btn-danger gradient ajax_page" title="Delete" escape="false">
    ...
    <script>
        $(document).ready(function(){
            ...
            $('#sendBack-btn').click(function(e){
                ...
                const id = $('#assign_button').data('id');
                let url = "{{ url('branchcode/send-back', '') }}";
                url += '/' + id;
            ...
            $('#approve-btn').click(function(){
                const id = $('#assign_button').data('id');
                let url = "{{ url('branchcode/approve', '') }}";
                url += '/' + id;
