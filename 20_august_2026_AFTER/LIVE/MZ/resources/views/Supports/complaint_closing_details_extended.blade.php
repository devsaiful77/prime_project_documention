...
@if(!empty($dataForView['extra_field']))
	...
	<tr>
        <th colspan="6"><div class="col-md-6" style="padding: 0;"> <h4>Issue Data</h4></div>
        	<div class="col-md-6 text-right"> <a href="#" data-reference="{{ $dataForView['reference_number'] }}" data-id="{{ $dataForView['main_id'] }}" data-toggle="modal" data-target="#issueHistoryModal" class="text-right"><i class="fa fa-list"></i> </a></div>
        </th>
	</tr>
	...
	@foreach($extra_fields as $key=>$r)
		...
		@foreach($r as $key1=>$value)
			...
			@endif
				...
				<th>{{ $key1 }}</th>
				<td @if($m_value=='true') style="background-color:#97333352" @endif>{{ (!empty($value))? $value:"" }}</td>
...
@if(!empty($dataForView['check_list']))
	...
	<tr>
		<th colspan="6"><div class="col-md-6" style="padding: 0;"> <h4>Check List</h4></div>
			@if(empty($dataForView['extra_field']))
			<div class="col-md-6 text-right"> <a href="#" data-reference="{{ $dataForView['reference_number'] }}" data-id="{{ $dataForView['main_id'] }}" data-toggle="modal" data-target="#issueHistoryModal" class="text-right"><i class="fa fa-list"></i> </a></div>
			@endif
		</th>	
