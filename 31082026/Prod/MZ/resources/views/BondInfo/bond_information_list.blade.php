            ...
            <tr>
                ...
                @if($checker == false)
                    <th class="vcenter text-center">
                        <a class="btn btn-success ajax_page gradient btn-sm" href="{{url('bond-info/upload')}}" title="Upload Product Info"><i class="fa fa-upload"></i> Upload Product Information</a>
                    </th>
                @endif 
        ...
        <tbody style="word-break: break-all;">
            ...
            @FOREACH($tblData as $key=>$data)
                <tr>
                    ...
                    <td class="vcenter text-center"> 
                        @IF($data['status'] == '0')
                            <a href="{{ url('/bond-info/status/'.$data['id']. '/1') }}" class="btn btn-info gradient btn-sm" title="Active" escape="false"> <i class="fa fa-check"></i> Active</a>
                        @ELSEIF($data['status'] == '1')
                            <?php
                                $editBtnLabel = '';
                                $editBtnCol = '';
                                
                                $editUrl = url('/bond-info/upload-edit/'.$data['id']);
                                $editBtnLabel = 'Edit Product Information';
                                $editBtnCol = 'success';
                            ...
                            ?>
                                ...
                                <a href="{{ url('/bond-info/status/'.$data['id']). '/0' }}" class="btn btn-danger gradient btn-sm" title="Inactive" escape="false"> <i class="fa fa-times"></i> Inactive</a>
