...
<style>
    ...
    {{-- <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <h5 class="modal-title" id="exampleModalLabel">Product Info Actions</h5>
...
<div class="table-responsive">
    <table class ="commonDataTableAllAsc table table-bordered table-striped table-hover">
        ...
        <tbody style="word-break: break-all;">
            @IF(!empty($productInfoTmp))
            @FOREACH($productInfoTmp as $data)
                <tr>
                    <td class="vcenter text-center"> {{ $data->category->name }} </td>
                    <td class="vcenter text-center"> {{ $data->subCategory->name }} </td>
                    <td class="vcenter text-center"> {{ $data->title }} </td>
                    <td class="vcenter text-center"> {{ $data->description }}</td>
                    <td class="vcenter text-center">
                        @if (!empty($data['file']))
                            <a target="_blank" href="{{ url('bond-info/preview/' . $data['id']) }}">{{ $data['file_name'] }}</a>
                        @else
                    ...
                    <td class="vcenter text-center">
                     ...
                     <td class="vcenter actions text-left">
                        ...
                        @if($data['form_status'] == 0)
                            <a href="{{ url('bond-info/assign', $data['id']) }}" class="btn btn-primary gradient ajax_page" title="Assign" escape="false"> Assign</a>
                        @else
                            @if($data['modified_by'] == Auth::user()->user_id)
                                <a href="#" class="btn btn-info gradient ajax_page" title="Approve" escape="false"> View</a>
                                <a href="{{ url('bond-info/approve', $data['id']) }}" class="btn btn-success gradient ajax_page" title="Approve" escape="false"> <i class="fa fa-plus"></i> Approve</a>
                                <a href="{{ url('bond-info/send-back', $data['id']) }}" class="btn btn-warning gradient ajax_page" title="Send Back" escape="false">SendBack To Maker</a>
                                <a href="{{ url('bond-info/reject', $data['id']) }}" class="btn btn-danger gradient ajax_page" title="Send Back" escape="false">Reject</a>
                            ...
                            {{-- <button class="btn btn-primary gradient ajax_page modal_button" type="button" data-id="{{ $data['id'] }}"  data-toggle="modal" data-target="#exampleModal">View</button> --}}
                                        @if($isChecker == false)
                                            @if($data['form_status'] == 7)
                                                <a href="{{ url('bond-info/tmp-edit', $data['id']) }}" class="btn btn-success gradient ajax_page" title="Edit" escape="false"> <i class="fa fa-pencil"></i> Edit</a>
                                            @endif
                                                <a href="{{ url('delete/tmp-data', ['id' => $data['id'], 'table' => 'bond_information_tmps']) }}" class="btn btn-danger gradient ajax_page" title="Delete" escape="false">
    ...
    <script>
        $(document).ready(function(){
            ...
            $('#sendBack-btn').click(function(e){
                ...
                const id = $('#assign_button').data('id');
                let url = "{{ url('bond-info/send-back', '') }}";
                url += '/' + id;
