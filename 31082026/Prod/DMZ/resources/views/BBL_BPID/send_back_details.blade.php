                ...
                @forelse($sendBackTickets as $status)
                        <tr>
                            ...
                            <td class="align-middle" style="width: 58%">
                                <span> <a href="{{route('BPID.send-back.ticket',['issueId'=>$status->serviceName->id, 'refNum'=>$status->reference_number, 'viewMode'=>'web', 'CIToken' => $ci_token, 'request_type' => 'service'])}}" class="text-decoration-underline">
                                        {{$status->serviceName->name}}
                                    </a></span> <br>
                                <span>
                                    <a href="{{route('BPID.send-back.ticket',['issueId'=>$status->serviceName->id, 'refNum'=>$status->reference_number, 'viewMode'=>'app', 'CIToken' => $ci_token, 'request_type' => 'service'])}}">{{$status->reference_number}}</a>
                                </span>
                            </td>
