        ...
        if ($this->file_name){
            foreach ($this->file_name as $index => $image) {
                if($image['is_required'] == 1){
                    /*$rules["file_name.$index.file"] = 'required|mimes_except:exe,bat,cmd|max:3072';*/
                    $rules["file_name.$index.file"] = 'required|mimes:jpeg,png,jpg,pdf,heif,heic|max:3072';
                }else {
                    $rules["file_name.$index.file"] = 'mimes:jpeg,png,jpg,pdf,heif,heic|max:3072';
                };
            }
        }
