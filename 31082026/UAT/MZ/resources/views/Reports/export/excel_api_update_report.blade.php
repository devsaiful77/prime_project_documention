...
<div class="table-responsive">
    <table class="table table-bordered table-condensed">
        ...
        <tr>
            <th style="" class="text-center vcenter wordwrap ">Ticket Number</th>
            @if( $searchDataForView["form_type"] != 'noncustomer')
                <th style="" class="text-center vcenter wordwrap ">Card / Acc Number</th>
            @endif
            <th style="" class="text-center vcenter wordwrap no-padding-margin-tb">Maker</th>
            @if($searchDataForView['date_type'] == 'action_date')
                <th style="" class="text-center vcenter nowrap ">Log Date</th>
                <th style="" class="text-center vcenter nowrap ">Log Time</th>
            @else
                <th style="" class="text-center vcenter nowrap ">Log Date</th>
                <th style="" class="text-center vcenter nowrap ">Log Time</th>
            @endif
            @if( $searchDataForView["form_type"] != 'noncustomer')
                <th style="" class="text-center vcenter wordwrap ">Customer Number</th>
                <th style="" class="text-center vcenter wordwrap ">Segment Code</th>
            @endif
            <th style="" class="text-center vcenter wordwrap ">Status</th>
            <th style="" class="text-center vcenter wordwrap ">API Updated By</th>
            <th style="" class="text-center vcenter wordwrap ">API Update Status</th>
            <th style="" class="text-center vcenter wordwrap ">API Update Date</th>
            <th style="" class="text-center vcenter wordwrap ">Failed Reason</th>
            <th style="" class="text-center vcenter wordwrap ">Cardpro Message</th>
            <th style="" class="text-center vcenter wordwrap ">Finacle Message</th>
            <th style="" class="text-center vcenter wordwrap ">Card Details</th>
            @if( $searchDataForView["form_type"] != 'noncustomer')
                <th style="" class="text-center vcenter wordwrap ">Product Type</th>
                @if( $searchDataForView["form_type"] == 'wform')
                    <th style="" class="text-center vcenter wordwrap ">Service Request Type</th>
                @else
                    <th style="" class="text-center vcenter wordwrap ">Complaint Type</th>
                @endif
            @endif
            @if($searchDataForView["form_type"] != 'noncustomer')
                        ...
                        @if(!empty($extraFieldArr))
                            @if($data['w_form_type'] != 1103 && $data['w_form_type'] != 1105)
                                @foreach($extraFieldArr AS $extFld)
                                    <th style="" class="text-center vcenter wordwrap ">{{ key($extFld) }}</th>
                                @endforeach
                            @endif
                        @endif
                    @endif
        ...
        @IF(!empty($dataForView))
            @FOREACH($dataForView AS $data)
                <?php
                $res = [];
                $res = getApiJsonResponse($data['reference_number']);
                ...
                if ($searchDataForView["form_type"] != 'noncustomer') {
                    ...
                    $getCommentInfo = getAPIUpdateActionComment($data['reference_number']);
                    $data['action_time'] = $getCommentInfo['action_time'];
                    $data['comments'] = $getCommentInfo['comments'];

                    //zihad 6-6-2023
                    $forExcelMassage = strip_tags($data['comments']);
                    $Finacle = '';
                    $Cardpro = '';
                    $a = explode(":", $forExcelMassage);
                    if (array_key_exists(0,$a)){
                        if(str_replace(' ', '', $a[0]) == 'Finacle'){
                            $c = explode("Cardpro", $a[1]);
                            if(array_key_exists(0,$c)){$Finacle = $c[0];}
                            if (array_key_exists(2,$a)){$Cardpro = $a[2];}
                        }
                        if(str_replace(' ', '', $a[0]) == "Cardpro"){
                            $c = explode("Finacle", $a[1]);
                            if(array_key_exists(0,$c)){$Cardpro = $c[0];}
                            if (array_key_exists(2,$a)){$Finacle = $a[2];}
                        }
                    }
                    //end
                ...
                @else
                    <tr>
                        ...
                        @IF( $searchDataForView["form_type"] == 'wform')
                            <td class="text-center vcenter no-padding-margin-tb">
                                <a href="{{ url('/Supports/WFormReportDetails/'.encrypt($data['reference_number'])) }}"
                                   target="_blank">{{ $data['reference_number'] }}</a>
                            </td>
                        @ELSEIF( $searchDataForView["form_type"] == 'complaint')
                            <td class="text-center vcenter no-padding-margin-tb">
                                <a href="{{ url('/Supports/ComplaintReportDetails/'.encrypt($data['reference_number'])) }}"
                                   target="_blank">{{ $data['reference_number'] }}</a>
                            </td>
                        @ELSEIF( $searchDataForView["form_type"] == 'noncustomer')
                            <td class="text-center vcenter no-padding-margin-tb">
                                <a href="{{ url('/Supports/NonCustomerReportDetails/'.encrypt($data['reference_number'])) }}"
                                   target="_blank">{{ $data['reference_number'] }}</a>
                            </td>
                        @ENDIF
                        @if( $searchDataForView["form_type"] != 'noncustomer')
                        ...
                        @endif
                        <td class="text-center vcenter no-padding-margin-tb">{{ (!empty($data['created_by_name'])) ? $data['created_by_name'] : $data['created_by'] }}
                           {{$data['source_maker']!=null? "(".$data['source_maker'].")" : ""}}
                        </td>
                        @if($searchDataForView['date_type'] == 'action_date')
                        ...
                        <td class="text-center vcenter no-padding-margin-tb">
                            {{ !empty($data['updated_by']) ? $data['updated_by'] .' ('. $data['name'] .')' : $data['name'] }}
                            &nbsp;[{{($data['unit_id']==1)? "Maker":"Checker"}}]
                        </td>
                        <td class="text-center vcenter no-padding-margin-tb">
                            {{ ($data['api_status'] == 1) ? "Success" : 'Failed' }}
                        </td>

                        <td class="text-center vcenter no-padding-margin-tb">
                            @if(!empty($data['action_time']))
                                {{ \Carbon\Carbon::createFromTimestamp($data['action_time'])->format('d-m-Y H:i:s') }}
                            @endif
                        </td>
                        <td class="text-center vcenter no-padding-margin-tb">
                        ...
                        @if( $searchDataForView["form_type"] != 'noncustomer')
                            ...
                            @if($searchDataForView['issueTypeExcel'] == 1)
                                @IF(!empty($extraFieldArr))
                                    @if($data['w_form_type'] != 1103 && $data['w_form_type'] != 1105)
                                        @FOREACH($extraFieldArr AS $extFld)
                                            <td style="" class="text-center vcenter no-padding-margin-tb">
                                                '{{ current($extFld) }}</td>
                                        @ENDFOREACH
                                    @endif
                                @ENDIF
                                ...
                                @IF(!empty($extraFieldArr))
                                    @if($data['w_form_type'] != 1103 && $data['w_form_type'] != 1105)
                                        @FOREACH($extraFieldArr AS $extFld)
                                            ...
                                            <td class="text-center vcenter no-padding-margin-tb">{{ key($extFld)}}
                                                :{{current($extFld)}}</td>
                                        @ENDFOREACH
                                    @else
                                        @php
                    ...
                    @ELSE
                        <tr>
                            <th class="text-center vcenter no-padding-margin-tb" colspan="38">Data not available</th>
                        </tr>
                    @ENDIF
        </tbody>
    </table>
