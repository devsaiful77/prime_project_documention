...
@if(!empty($dataForView['w_form_type']['extra_field']))
    ...
    @if(!empty($dataForView['main_id'] == 1103) || !empty($dataForView['main_id'] == 1105))
            ...
            @foreach($r as $key1=>$value)
                ...
                @if($i==1)
                    <tr>
                        ...
                        @php //$a = get_service_request_value($dataForView['main_id']); @endphp
                        @php //$b = get_service_request_label($dataForView['main_id']); @endphp

                        <th>{{ $key1 }}</th>
                        <td @if($m_value=='true') style="background-color:#97333352" @endif>{{ (isset($value)) ? (is_array($value) ? implode(', ', $value) : $value ) :"" }}</td>
        ...
        @foreach($r as $key1=>$value)
            ...
            @if($j==1)
                <tr>
                    ...
                    <th>{{ $key1 }}</th>
                    <td @if($cl_value=='true') style="background-color:#97333352" @endif>{{ (isset($value))? $value:"No" }}</span></td>
