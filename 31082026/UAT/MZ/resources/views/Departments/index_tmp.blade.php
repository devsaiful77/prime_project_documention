    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Department Actions</h5>
...
<div class="table-responsive">
    <table class ="commonDataTableAllAsc table table-bordered table-striped table-hover">
        ...
        <tbody style="word-break: break-all;">
            @IF(!empty($departmentTmpData))
            @FOREACH($departmentTmpData as $key=>$data)
                <tr>
                    ...
                    <td class="vcenter actions text-center">
                        {{-- @if($data['form_status'] == 0)
                         <button type="button" class="btn btn-primary assignBtn" data-toggle="modal" data-target="#comparisonModal"
                        data-id="{{ $data->id }}" data-name="{{ $data->name }}">Assign</button>


                        @else
                            @if($data['modified_by'] == Auth::user()->user_id)
                                <a href="#" class="btn btn-info gradient ajax_page" title="Approve" escape="false"> View</a>
                                <a href="{{ url('/Departments/approve', $data['id']) }}" class="btn btn-success gradient ajax_page" title="Approve" escape="false"> <i class="fa fa-plus"></i> Approve</a>
                                <a href="{{ url('Departments/send-back', $data['id']) }}" class="btn btn-warning gradient ajax_page" title="Send Back" escape="false">SendBack To Maker</a>
                                <a href="{{ url('Departments/reject', $data['id']) }}" class="btn btn-danger gradient ajax_page" title="Reject" escape="false">Reject</a>
                            @else
                        ...
                        @if($isChecker == false)
                            @if($data['form_status'] == 7)
                                <a href="{{ url('Departments/tmp-edit', $data['id']) }}" class="btn btn-success gradient ajax_page" title="Edit" escape="false"> <i class="fa fa-pencil"></i> Edit</a>
                            @endif
                                <a href="{{ url('delete/tmp-data', ['id' => $data['id'], 'table' => 'departments_tmp']) }}" class="btn btn-danger gradient ajax_page" title="Delete" escape="false">
    ...
    <script>
        $(document).ready(function(){
            ...
            $('#sendBack-btn').click(function(e){
                ...
                const id = $('#assign_button').data('id');
                let url = "{{ url('Departments/send-back', '') }}";
                url += '/' + id;
            ...
            $('#reject-btn').click(function(e){
                ...
                const id = $('#assign_button').data('id');
                let url = "{{ url('Departments/reject', '') }}";
                url += '/' + id;
            ...
            $('#approve-btn').click(function(){
                const id = $('#assign_button').data('id');
                let url = "{{ url('Departments/approve', '') }}";
                url += '/' + id;
